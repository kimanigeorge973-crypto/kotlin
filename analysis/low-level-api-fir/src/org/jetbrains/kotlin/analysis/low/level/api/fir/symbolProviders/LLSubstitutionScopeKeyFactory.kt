...
internal class LLSubstitutionScopeKeyFactory(private val session: LLFirSession) : SubstitutionScopeKeyFactory {
    ...
    private fun recordAnnotations(type: ConeKotlinType, stack: Stack, consumer: MutableList<TypeArgumentAnnotation>) {
        for (typeAnnotation in type.typeAnnotations) {
            val lookupTag = typeAnnotation.annotationTypeRef.coneTypeOrNull?.classLikeLookupTagIfAny
    ...
    private class Stack {
        ...
        fun createAnnotation(lookupTag: ConeClassLikeLookupTag?): TypeArgumentAnnotation {
            if (size < 4) {
                ...
                val path = IntArray(size) { stack[it] }
    ...
    private class DeepTypeArgumentAnnotation(
        ...
        override fun hashCode(): Int {
            return 31 * lookupTag.hashCode() + path.contentHashCode()
        }
