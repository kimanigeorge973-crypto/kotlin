...
internal fun equals(obj1: dynamic, obj2: dynamic): Boolean {
    ...
    if (isCallableReference(obj1) && isCallableReference(obj2)) {
        ...
        if (bound1.size != bound2.size) return false

        for (i in 0..<bound1.size)
            if (!equals(bound1[i], bound2[i]))
                return false
...
@UsedFromCompilerGeneratedCode
internal fun getCallableReferenceHashCode(obj: dynamic): Int {
