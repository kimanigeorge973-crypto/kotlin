...
object C {
    @JvmStatic
    fun foo(x: S, y: Int, z: S?): S = x + S(y) + z!!
...
fun box(): String {
    ...
    assertEquals(seven, cMembers.getValue("staticDefault1_2").callBy(C, seven))
    ...
    assertEquals(seven, C::foo.callBy(one, 2, four))
    assertEquals(seven, C::staticDefault1_1.callBy(seven))
    assertEquals(default, C::staticDefault1_1.callBy(emptyMap()))
    assertEquals(seven, C::staticDefault1_2.callBy(seven))
