...
class DeclarationGenerator(
    ...
    fun generateFunction(declaration: IrFunction) {
        ...
        val importedName = when {
            wasmImportModule != null -> {
                ...
                linkerDataContext.addJsModuleImport(declaration.symbol, wasmImportModule.moduleName)
                wasmImportModule
            }
            jsBuiltin != null -> {
                check(declaration.isExternal) { "Non-external fun with @JsBuiltin ${declaration.fqNameWhenAvailable}"}
                linkerDataContext.addJsModuleImport(declaration.symbol, jsBuiltin.moduleName)
                linkerDataContext.addJsBuiltin(jsBuiltin.declarationName, jsBuiltin.polyfillImpl)
    ...
    private fun createRtti(metadata: ClassMetadata) {
        ...
        if (backendContext.isWasmJsTarget) {
            packageNameStringLiteralId = linkerDataContext.referenceGlobalStringId(qualifier)
            simpleNameStringLiteralId = linkerDataContext.referenceGlobalStringId(simpleName)
        } else {
            packageNameStringLiteralId = linkerDataContext.referenceStringLiteralId(qualifier)
            simpleNameStringLiteralId = linkerDataContext.referenceStringLiteralId(simpleName)
        }
        ...
        val initRttiGlobal = buildWasmExpression {
            ...
            buildConstI64(linkerDataContext.referenceTypeId(symbol), location)
            ...
            if (backendContext.isWasmJsTarget) {
                val packageNameGlobalReference = linkerDataContext.referenceGlobalStringGlobal(qualifier)
                buildGetGlobal(packageNameGlobalReference, location)

                val simpleNameGlobalReference = linkerDataContext.referenceGlobalStringGlobal(simpleName)
    ...
    private fun interfaceTable(builder: WasmExpressionBuilder, classMetadata: ClassMetadata, location: SourceLocation) {
        ...
        for (iFace in supportedPushedBack) {
            builder.buildConstI64(linkerDataContext.referenceTypeId(iFace.symbol), location)
        }
    ...
    when (val kind = expression.kind) {
        ...
        is IrConstKind.String -> {
            ...
            if (backendContext.isWasmJsTarget && !stringValue.hasUnpairedSurrogates) {
                val literalIdToStore = linkerDataContext.referenceGlobalStringId(stringValue)
                ...
                if (stringValue.length > MAX_WASM_IMPORT_NAME_LENGTH) {
                    ...
                    val globalReferenceFirst = linkerDataContext.referenceGlobalStringGlobal(stringValueSplits.first())
                    body.buildGetGlobal(globalReferenceFirst, location)

                    for (stringValueSplit in stringValueSplits.drop(1)) {
                        val globalReference = linkerDataContext.referenceGlobalStringGlobal(stringValueSplit)
                        body.buildGetGlobal(globalReference, location)
                        body.buildCall(jsConcat, location)
                    }
                } else {
                    val globalReferenceFirst = linkerDataContext.referenceGlobalStringGlobal(stringValue)
                    body.buildGetGlobal(globalReferenceFirst, location)
                }

                body.buildCall(Synthetics.Functions.createStringLiteralJsString, location)
            } else {
                val literalId = linkerDataContext.referenceStringLiteralId(stringValue)
