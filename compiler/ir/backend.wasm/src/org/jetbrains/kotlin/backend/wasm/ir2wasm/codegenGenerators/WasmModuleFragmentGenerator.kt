...
fun compileIrFile(
    ...
    irFile.declarations.forEach {
        ...
        if (it is IrClass && it.origin == WebCallableReferenceLowering.FUNCTION_REFERENCE_IMPL) {
            val equivalenceKey = it.name.asString()
            linkerDataContext.addEquivalentGcType(equivalenceKey, it.symbol)
            linkerDataContext.addEquivalentVTableGcType(equivalenceKey, it.symbol)
            linkerDataContext.addEquivalentRttiGlobal(equivalenceKey, it.symbol)
            linkerDataContext.addEquivalentVTableGlobal(equivalenceKey, it.symbol)
            linkerDataContext.addEquivalentClassITableGlobal(equivalenceKey, it.symbol)
        }
    }
